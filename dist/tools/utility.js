...
class UtilityTools {
    ...
    // ---------------------------------------------------------------------------
    createTemplatesListTool() {
        return {
            name: 'bookstack_templates_list',
            description: 'List all pages that are marked as templates in BookStack. ' +
    ...
    // ---------------------------------------------------------------------------
    createContentPathTool() {
        return {
            name: 'bookstack_content_path',
            description: 'Navigate upward through the BookStack content hierarchy for any page or chapter. ' +
            ...
            handler: async (params) => {
                ...
                const contentId = this.validator.validateId(params.content_id);
    ...
    // ---------------------------------------------------------------------------
    createContentSummarizeTool() {
        return {
            name: 'bookstack_content_summarize',
            description: 'Generate a compact structural summary of a book or chapter without fetching every page\'s full content. ' +
            ...
            handler: async (params) => {
                const contentType = params.content_type;
                const contentId = this.validator.validateId(params.content_id);
                const includeTags = params.include_tags ?? true;
                ...
                if (contentType === 'book') {
                    ...
                    const topLevelPages = contents.filter((c) => c.type === 'page');
    ...
    // ---------------------------------------------------------------------------
    createAuditLogSummaryTool() {
        return {
            name: 'bookstack_audit_log_summary',
            description: 'Aggregate audit log entries into a human-readable activity summary over a date range. ' +
            ...
            handler: async (params) => {
                ...
                for (const entry of filtered) {
                    ...
                    const userName = entry.user?.name ?? `user:${entry.user_id}`;
    ...
    // ---------------------------------------------------------------------------
    createRateLimitStatusTool() {
        return {
            name: 'bookstack_ratelimit_status',
            description: 'Return the current state of the API rate-limiter token bucket. ' +
