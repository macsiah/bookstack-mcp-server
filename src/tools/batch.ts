...
export class BatchTools {
  ...
  private createBatchCreateContentTool(): MCPTool {
    return {
      name: 'bookstack_batch_create_content',
      description:
        'Create multiple pages and/or chapters in a single call. ' +
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          try {
            ...
            if (item.type === 'chapter') {
              ...
              if (item.book_id) pageParams.book_id = Number(item.book_id);
              if (item.chapter_id) pageParams.chapter_id = Number(item.chapter_id);
